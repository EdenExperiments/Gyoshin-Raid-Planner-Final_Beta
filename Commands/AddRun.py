...
async def AddRunInDM(message, bot):
  ...
  try:
    ...
    ChannelID = message.channel.id
     ...
     await DMHelper.DMUser(message, f"Something went wrong when gathering server and user information.")
     return
  ...
  await DMHelper.DMUser(message, f"Hi {CreatorDisplay}, let's get you forming your crew in {GuildName}.\nFirst, give me a brief description for your crew, e.g. 'Friday Night Alliance Raid'.\n")
  try:
    response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck)
  except:
    await DMHelper.DMUser(message, "Your request has timed out, please call the command again from a server if you still wish to add a run.")
    return   
  ...
  while not DateTime:
    try:
      await DMHelper.DMUser(message, f"Now, please tell me the date and time of your run in the format of 'dd-mm-yyyy hh:mm'.")
      response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck) 
    ...
    try:
      ...
      if not sqldatetime:
        await DMHelper.DMUser(message, f"Invalid date and time detected, please use the dd-mm-yyyy hh:mm format not sql")
        continue
    except:
      await DMHelper.DMUser(message, f"Invalid date and time detected, please use the dd-mm-yyyy hh:mm format  exception")
      continue
    
    try:
      DateTime = response.content
    except:
      await DMHelper.DMUser(message, "Error saving DateTime")
  ...
  if row:  
    while not UsingTemplate:
      try:
        await DMHelper.DMUser(message, f"Do you wish to use a template (Y/N)?")
        response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck)
      except asyncio.TimeoutError:
        await DMHelper.DMUser(message, "Your request has timed out, please call the command again from a server if you still wish to add a run.")
        return
      ...
      if response.content == "Y" or response.content == "y" or response.content == "Yes" or response.content == "yes":
        ...
        await DMHelper.DMUser(message, f"Please enter a valid response of 'yes' or 'no'.")
  else:
    UsingTemplate = "no"
  ...
  if UsingTemplate == 'yes':
    ...
    while template_completion == False:
      try:
        await DMHelper.DMUser(message, f"Which template would you like to use from the list above? Please respond with the name of the template.")
        response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck)
      except asyncio.TimeoutError:
        await DMHelper.DMUser(message, "Your request has timed out, please call the command again from a server if you still wish to add a run.")
        return
      ...
      try:
        ...
        if not row:
          await DMHelper.DMUser(message, f"I could not find the template {Template} on this server, please ensure name is correct.")
          conn.close()
          continue
      except:
        await DMHelper.DMUser(message, f"Something went wrong checking for template, please try again.")
        conn.close()
        continue
      ...
      try:        
        ...
        await DMHelper.DMUser(message, f"Something went wrong obtaining the player and/or role numbers from the template, please try again.")
        conn.close()   
        continue
  ...
  # Code for if user is not using a template
    ...
    while variable_completion == False:
      ...
      while PlayerLoop == False:
        try:
          await DMHelper.DMUser(message, f"What is the total number of players for your crew? Please enter a number greater than 0.")
          response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck)
          try:
            NrOfPlayers = int(response.content)
          except:
            await DMHelper.DMUser(message, "Please enter a valid integer.")
            continue
        except asyncio.TimeoutError:
          await DMHelper.DMUser(message, "Your request has timed out, please call the command again from a server if you still wish to add a run.")
          return
        ...
        if NrOfPlayers <= 0:
          await DMHelper.DMUser(message, "Please enter a valid number greater than 0.")
          continue
      ...
      while TankLoop == False:
        try:
          await DMHelper.DMUser(message, f"What is the total number of tanks for your crew? Please enter a number equal or greater than 0.")
          response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck)
          try:
            NrOfTanks = int(response.content)
          except:
            await DMHelper.DMUser(message, "Please enter a valid number.")
            continue
        except asyncio.TimeoutError:
          await DMHelper.DMUser(message, "Your request has timed out, please call the command again from a server if you still wish to add a run.")
          return
      ...
      while HealerLoop == False:
        try:
          await DMHelper.DMUser(message, f"What is the total number of healers for your crew? Please enter a number equal or greater than 0.")
          response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck)
          try:
      ...
      while DpsLoop == False:
        try:
          await DMHelper.DMUser(message, f"What is the total number of dps for your crew? Please enter a number equal or greater than 0.")
          response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck)
          try:
  ...
  while not RoleName: 
    try:
      await DMHelper.DMUser(message, f"Next, which role (tank/dps/healer) do you intend to play as within the crew?")
      response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck)
    except asyncio.TimeoutError:
      await DMHelper.DMUser(message, "Your request has timed out, please call the command again from a server if you still wish to add a run.")
      return       
    ...
    try:     
      ...
      await DMHelper.DMUser(message, f"Invalid role, please enter a valid role, you can call !roles in the servers Gyoshin channel to have available roles sent to this DM.")
      continue
    ...
    try:
      if response.content == "tank":
        if NrOfTanks <= 0:
          await DMHelper.DMUser(message, "You have entered the role of tank, number of tanks must be greater than 0 for you to pick this role. Please pick a different role.")
        else:
          NumberOfCurrentTanks = 1
          RoleName = response.content
      elif response.content == "dps":
        if NrOfDps <= 0:
          await DMHelper.DMUser(message, "You have entered the role of dps, number of dps must be greater than 0 for you to pick this role. Please pick a different role.")
        else:
          NumberOfCurrentDps = 1
          RoleName = response.content
      elif response.content == "healer":
        if NrOfHealers <= 0:
          await DMHelper.DMUser(message, "You have entered the role of healer, number of healers must be greater than 0 for you to pick this role. Please pick a different role.")
        else:
      ...
      await DMHelper.DMUser(message, "Error in checking role selected to number of role slots available.")
  ...
  while final_confirmation == False:
    try:
      await DMHelper.DMUser(message, f"Please confirm that you wish to create a crew with the following details (Y/N): \n**Description**: {Name}\n**Date:** {DateTime}\n**Number of Tanks:** {NrOfTanks}\n**Number of Healers:** {NrOfHealers}\n**Number of DPS:** {NrOfDps}")
      response = await bot.wait_for(event='message' ,timeout = 30, check= DMCheck)
    except asyncio.TimeoutError:
      await DMHelper.DMUser(message, "Your request has timed out, please call the command again from a server if you still wish to add a run.")
    ...
    if response.content == "Y" or response.content == "y" or response.content == "Yes" or response.content == "yes":
      ...
      await DMHelper.DMUser(message, f"Your request to create a crew has been cancelled, please call the command again in the relevant channel if you wish to try again.")
      return
    else:
      continue
